  ...
  tabLinks.forEach((tabLink) => {
    tabLink.addEventListener("click", (e) => {
      ...
      projectContents.forEach((projectContent) => {
        // 클래스에 따라 컨텐츠 표시
        projectContent.style.display = projectContent.classList.contains(
          targetTab
        )
          ? "block"
          : "none";
  ...
  tabLinks.forEach((tabLink) => {
    tabLink.addEventListener("click", (e) => {
      ...
      helpContents.forEach((helpContent) => {
        // 클래스에 따라 컨텐츠 표시
        helpContent.style.display = helpContent.classList.contains(targetTab)
          ? "flex"
          : "none";
      });
    });
  ...
  const changeImg = () => {
    helpImgs.forEach((helpImg, index) => {
      if (window.innerWidth > 767 && window.innerWidth < 1023) {
        if (index === 0) {
          helpImg.src =
            "https://t1.kakaocdn.net/thumb/C630x354.fwebp.q100/?fname=https%3A%2F%2Ft1.kakaocdn.net%2Fkakaocorp%2Fkakaocorp%2Fservice%2Fdangol%2Fimg_help01_768.png";
        } else if (index === 1) {
          helpImg.src =
            "https://t1.kakaocdn.net/thumb/C630x354.fwebp.q100/?fname=https%3A%2F%2Ft1.kakaocdn.net%2Fkakaocorp%2Fkakaocorp%2Fservice%2Fdangol%2Fimg_help02_768.png";
        } else if (index === 2) {
          helpImg.src =
            "https://t1.kakaocdn.net/thumb/C630x354.fwebp.q100/?fname=https%3A%2F%2Ft1.kakaocdn.net%2Fkakaocorp%2Fkakaocorp%2Fservice%2Fdangol%2Fimg_help03_768.png";
        }
      }
    });
  };
...
const showShareNews = () => {
  ...
  btnShares.forEach((btnShare) => {
    btnShare.addEventListener("click", () => {
      ...
      const associatedLayer = btnShare
        .closest(".item-card-new")
        .querySelector(".layer-share");
